 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
     ...
     */
     ...
     // Cost of minimize the use of actuators
     for(int t = 0;t<N-1;++t){
       fg[0] += 100000 * CppAD::pow( vars[delta_start + t], 2 );
       fg[0] += CppAD::pow( vars[a_start + t], 2 );
     }

     // Cost of minimize the value gap between sequential actuations
     for(int t = 0;t<N-2;++t){
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      AD<double> f0 = coeffs[0] + coeffs[1] * x0 + coeffs[2]*x0*x0 + coeffs[3]*x0*x0*x0; // refere trajectory
...
std::vector<double> MPC::Solve(const VectorXd &state, const VectorXd &coeffs) {
  ...
  // average the first three actuations
  double delta = 0.0;
  double a = 0.0;
